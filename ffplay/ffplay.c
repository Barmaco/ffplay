...
typedef struct PacketQueue			//待解码包的队列
{
...
typedef struct VideoPicture		//显示的画面
{
    SDL_Overlay *bmp;
    int width, height; // source height & width
} VideoPicture;

typedef struct VideoState		//主对象，保存各种信息，方便随时取用
{
    ...
    int audio_stream; 
    int video_stream;			//正在播放的音视频的索引

    AVStream *audio_st;
    AVStream *video_st;			//正在播放的音视频的流

    PacketQueue audioq;     //音频待解码队列
    PacketQueue videoq;		//视频待解码队列

    VideoPicture pictq[VIDEO_PICTURE_QUEUE_SIZE];	//解码后的图像对象，准备显示
    double frame_last_delay;					//视频图像显示间隔时间

    uint8_t audio_buf[(AVCODEC_MAX_AUDIO_FRAME_SIZE *3) / 2];   //输入的音频缓存
    unsigned int audio_buf_size;
    int audio_buf_index;
    AVPacket audio_pkt;
    uint8_t *audio_pkt_data;				//音频数据包首地址
...
int64_t av_gettime(void)   //取得当前时间 从1970-01-01到现在的微妙数
{
...
static void packet_queue_flush(PacketQueue *q)  //清空队列 释放队列中所有对象
{
...
static void packet_queue_end(PacketQueue *q)   //删掉这个队列
{
...
static int packet_queue_put(PacketQueue *q, AVPacket *pkt)  //把一个包插入队列，挂在尾部
{
    AVPacketList *pkt1;
...
static int packet_queue_get(PacketQueue *q, AVPacket *pkt, int block)  //从队列中取包，每次取一包 
{
    AVPacketList *pkt1;
    int ret;
    ...
    for (;;)
    {
        ...
        if (pkt1)
        {
            ...
            q->size -= pkt1->pkt.size;
            *pkt = pkt1->pkt;
            av_free(pkt1);     //free的是 AVPacketList 而不是 AVPacket 该数据已经在pkt中
            ret = 1;
            break;
        }
...
static void alloc_picture(void *opaque)   //创建一个显示表面
{
...
static int video_display(VideoState *is, AVFrame *src_frame, double pts)
{
    ...
    if (vp->bmp)
    {
        ...
        if (pts)
            Sleep((int)(is->frame_last_delay *1000));  //简单的延迟显示
#if 1
        /* get a pointer on the bitmap */
        SDL_LockYUVOverlay(vp->bmp);    //使用前先锁定该表面（YUV格式的数据）
        ...
        img_convert(&pict, 
                    ...
                    is->video_st->actx->height);  //将frame中数据转化格式  放入pict.data中 也就是vp->bmp中
...
static int decode_thread(void *arg)
{
    ...
    for (i = 0; i < ic->nb_streams; i++)
    {
        ...
        case CODEC_TYPE_VIDEO:
            ...
            screen = SDL_SetVideoMode(enc->width, enc->height, 0, flags);// 修改是为了适配视频大小

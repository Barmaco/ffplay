...
AVInputFormat *av_probe_input_format(AVProbeData *pd, int is_opened)
{
    ...
    fmt = NULL;
    score_max = 0;
    for (fmt1 = first_iformat; fmt1 != NULL; fmt1 = fmt1->next)  //一个一个的尝试，看看每个格式是否符合
    {
        if (!is_opened)
            continue;
        ...
        if (fmt1->read_probe)
        {
            score = fmt1->read_probe(pd);//尝试正确 就是avi 返回100
        }
        else if (fmt1->extensions)   //失败，看看扩展名是否符合
        {
					   ...
					   int buf_size, AVFormatParameters *ap)
{
    ...
    {
        if (url_fopen(pb, filename, URL_RDONLY) < 0) //打开这个文件或者是网络端口 并初始化pb1
        {
            err = AVERROR_IO;
            goto fail;
        }
		...
		for (probe_size = PROBE_BUF_MIN; probe_size <= PROBE_BUF_MAX && !fmt; probe_size <<= 1)  //先读PROBE_BUF_MIN(2048)字节文件开始数据识别文件格式，
																								 //如果不能识别文件格式，就把识别文
																								 //件缓存以2 倍的增长扩大再读文件开始数据识别，直到识别出文件格式或者超过131072 字节缓存。
        {
            pd->buf = av_realloc(pd->buf, probe_size);
            pd->buf_size = url_fread(pb, pd->buf, probe_size);
            if (url_fseek(pb, 0, SEEK_SET) == (offset_t) - EPIPE) //读取之后把文件指针复位，下次重新读的时候用
            {
                url_fclose(pb);
            ...
            fmt = av_probe_input_format(pd, 1);	//开始识别封装格式（需要的线索就在pd->buf）
        }
        av_freep(&pd->buf);
    }
